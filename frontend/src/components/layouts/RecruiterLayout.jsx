      ...
      <div className="header-wrapper">
        ...
        <nav className="menu">
          <ul className={`nav-links ${isMenuOpen ? 'active' : ''}`} id="navLinks">
            ...
            {Object.values(rMenu).filter(item => item.class === 'links').map((menuItem) => (
              <li key={menuItem.id}>
            ...
            <li className="mobile-buttons">
              {Object.values(rMenu).filter(item => item.class === 'button').map((menuItem) => (
                <Link key={menuItem.id} href={menuItem.link} className="headbtn btn btn-outline">
                  {menuItem.title}
                </Link>
              ))}
            </li>
